		...
		switch (com)
		{
			case create:
				...
				if (args.length == 5)
				{
					...
					this.salaryManager.createSalary(sender, name, salary,
							interval, employer);
				}
			...
			case apply:
				if (args.length >= 4)
				{
					if (args[1].equals("group"))
					{
						...
						{
							this.salaryManager.addGroups(sender, args[2],
									args[3]);
						}
						else
						{
			...
			case remove:
				if (args.length >= 4)
				{
					if (args[1].equals("group"))
					{
						if (PermissionsManager.pluginDetected())
						{
							this.salaryManager.removeGroups(sender, args[2],
									args[3]);
						}
						else
						{
					...
					{
						this.salaryManager.removePlayers(sender, args[2],
								args[3]);
					}
					sender.sendMessage(ChatColor.RED + CashFlow.TAG
							+ "Incorrect argument.");
			...
			case removeexception:
				...
				{
					this.salaryManager
							.removeException(sender, args[1], args[2]);
				}
				else
				{
					sender.sendMessage(ChatColor.RED + CashFlow.TAG
				...
				if (args.length == 3 || args.length == 4)
				{
							...
							|| booleanString.equals("false"))
					{
						...
						sender.sendMessage(ChatColor.GREEN + CashFlow.TAG
								+ "Online only set to " + ChatColor.GRAY
								+ args[1]);
					}
					else
					{
			...
			case fire:
				if (args.length == 2)
				{
					...
					{
						sender.sendMessage(LocalString.INVALID_NAME
								.parseString(info));
					}
				}
				else
				...
				if (args.length == 2)
				{
							...
							.contains(salaryName))
					{
						...
						sender.sendMessage(ChatColor.YELLOW + CashFlow.TAG
								+ " Disabling salary - " + ChatColor.AQUA
								+ salaryName);
