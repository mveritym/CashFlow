...
public class CommandManager {
	...
	public CommandManager(CashFlow cashFlow, TaxManager taxManager,
			...
			case remove:
				if (args.length >= 3)
				{
					if (args[0].equals("group"))
					{
						if (this.cashFlow.permsManager.pluginDetected())
						{
							this.taxManager.removeGroups(sender, args[1],
									args[2]);
							return true;
						}
						else
			...
			case setonlineonly:
				if (args.length == 2 || args.length == 3)
				{
					if (args[1].equals("true") || args[1].equals("false"))
					{
						...
						this.taxManager.setOnlineOnly(args[0],
								Boolean.parseBoolean(args[1]), onlineInterval);
			...
			case enable:
				if (args.length == 1)
				{
					...
					if (cashFlow.getPluginConfig().getStringList("taxes.list")
							.contains(taxName))
					{
						sender.sendMessage(ChatColor.GREEN + "Enabling tax - "
	...
	public boolean salaryCommand(CommandSender sender, String[] tempArgs) {
		...
		switch (cmd)
		{
			...
			case apply:
				if (args.length >= 3)
				{
					if (args[0].equals("group"))
					{
						if (this.cashFlow.permsManager.pluginDetected())
						{
							this.salaryManager.addGroups(sender, args[1],
									args[2]);
							return true;
						}
						else
			...
			case remove:
				if (args.length >= 3)
				{
					if (args[0].equals("group"))
					{
						if (this.cashFlow.permsManager.pluginDetected())
						{
							this.salaryManager.removeGroups(sender, args[1],
									args[2]);
							return true;
						}
						else
					...
					{
						this.salaryManager.removePlayers(sender, args[1],
								args[2]);
						return true;
					}
					sender.sendMessage(ChatColor.RED + "Incorrect argument.");
			...
			case removeexception:
				...
				{
					this.salaryManager
							.removeException(sender, args[0], args[1]);
					return true;
				}
				else
				{
			...
			case setonlineonly:
				if (args.length == 2 || args.length == 3)
				{
					if (args[1].equals("true") || args[1].equals("false"))
					{
						...
						this.salaryManager.setOnlineOnly(args[0],
								Boolean.parseBoolean(args[1]), onlineInterval);
						sender.sendMessage(ChatColor.GREEN
								+ "Online only set to " + args[1]);
						return true;
