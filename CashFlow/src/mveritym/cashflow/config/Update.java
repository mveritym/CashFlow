...
public class Update {
   ...
   public static void update() {
      ...
      if(ver < 1.1) {
         ...
         plugin.getLogger().info(
               CashFlow.TAG + " Altering cashflow table to add laston column");
         query = "ALTER TABLE cashflow ADD laston REAL;";
         plugin.getDatabaseHandler().standardQuery(query);
         // Update table to add check column
         plugin.getLogger().info(
               CashFlow.TAG + " Altering cashflow table to add check column");
      ...
      if(ver < 1.12) {
         ...
         query = "ALTER TABLE cashflow RENAME TO " + Table.CASHFLOW.getName()
               + ";";
         plugin.getDatabaseHandler().standardQuery(query);
         plugin.getLogger().info(
      ...
      if(ver < 1.184) {
         ...
         for(String salaryName : salaries) {
            plugin.getConfig().set("salaries." + salaryName + ".autoEnable",
                  true);
            plugin.saveConfig();
         }
      }
