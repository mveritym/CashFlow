   ...
   private void checkTables() {
      if(useMySQL) {
         ...
         if(!mysql.checkTable(Table.CASHFLOW.getName())) {
            plugin.getLogger()
                  .info(CashFlow.TAG + " Created master list table");
            // Master table
   ...
   private void importSQL() {
      ...
      try {
         ...
         sqlite = new SQLite(plugin.getLogger(), CashFlow.TAG, "database",
               plugin.getDataFolder().getAbsolutePath());
         // Copy items
         Query rs = sqlite.select("SELECT * FROM " + Table.CASHFLOW.getName()
               + ";");
         if(rs.getResult().next()) {
            plugin.getLogger()
                  .info(CashFlow.TAG + " Importing master table...");
            do {
         ...
         plugin.getLogger().warning(CashFlow.TAG + " SQL Exception on Import");
         e.printStackTrace();
      }
