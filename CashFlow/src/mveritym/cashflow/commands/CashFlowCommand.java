...
public class CashFlowCommand implements CommandExecutor {
   ...
   private void parseCommand(CFCommand com, CommandSender sender,
      ...
      case restart:
         ...
         this.salaryManager.enable();
         Buffer.getInstance().start();
         sender.sendMessage(LocalString.COMMAND_CASHFLOW_RESTART
               .parseString(info));
         break;
      case setworld:
         if(args.length == 2) {
            if(PermissionsManager.setWorld(args[1])) {
               ...
               sender.sendMessage(ChatColor.YELLOW + CashFlow.TAG + " World "
                     + ChatColor.GRAY + args[1] + ChatColor.YELLOW
                     + " not found.");
            }
         } else {
         ...
         if(args.length == 2) {
            ...
            if(w != null) {
               ...
               sender.sendMessage(ChatColor.YELLOW + CashFlow.TAG
                     + " Importing players of world '" + ChatColor.GRAY
                     + worldName + ChatColor.YELLOW
      ...
      case status:
         ...
         for(CFTask t : this.taxManager.taxTasks) {
            sender.sendMessage(ChatColor.RED + t.getName() + ChatColor.GRAY
                  + " : " + t.getState());
         }
         for(CFTask t : this.salaryManager.salaryTasks) {
            sender.sendMessage(ChatColor.GREEN + t.getName() + ChatColor.GRAY
                  + " : " + t.getState());
         }
