...
public class SalaryCommand implements CommandExecutor {
   ...
   public SalaryCommand(CashFlow plugin, SalaryManager salary) {
      this.plugin = plugin;
   ...
   private void parseCommand(CFCommand com, CommandSender sender,
      ...
      case create:
         ...
         if(args.length == 5) {
            ...
            this.salaryManager.createSalary(sender, name, salary, interval,
                  employer);
         ...
         if(args.length >= 4) {
            if(args[1].equals("group")) {
               if(PermissionsManager.pluginDetected()) {
                  this.salaryManager.addGroups(sender, args[2], args[3]);
               } else {
                  sender.sendMessage(ChatColor.RED
                        + " You must install a permissions plugin to use this command.");
               }
            } else if(args[1].equals("player")) {
               this.salaryManager.addPlayers(sender, args[2], args[3]);
            }
         } else {
            sender.sendMessage(ChatColor.RED
                  + " Incorrect number of arguments.");
      ...
      case remove:
         if(args.length >= 4) {
            if(args[1].equals("group")) {
               if(PermissionsManager.pluginDetected()) {
                  this.salaryManager.removeGroups(sender, args[2], args[3]);
               } else {
                  sender.sendMessage(ChatColor.RED
                        + CashFlow.TAG
                        + " You must install a permissions plugin to use this command.");
               ...
               this.salaryManager.removePlayers(sender, args[2], args[3]);
            }
            sender.sendMessage(ChatColor.RED + CashFlow.TAG
                  + "Incorrect argument.");
         } else {
      ...
      case addexception:
         if(args.length == 3) {
            this.salaryManager.addException(sender, args[1], args[2]);
         } else {
            sender.sendMessage(ChatColor.RED + CashFlow.TAG
                  + " Incorrect number of arguments.");
         }
      ...
      case removeexception:
         if(args.length == 3) {
            this.salaryManager.removeException(sender, args[1], args[2]);
         } else {
            sender.sendMessage(ChatColor.RED + CashFlow.TAG
                  + " Incorrect number of arguments.");
         }
      ...
      case fire:
         if(args.length == 2) {
            String salaryName = args[1];
            info.put(Flag.EXTRA, salaryName);
            if(plugin.getPluginConfig().getStringList("salaries.list")
                  .contains(salaryName)) {
               this.plugin.getLogger().info(
                     CashFlow.TAG + " Paying salary " + salaryName);
      ...
      case enable:
         if(args.length == 2) {
            ...
            if(plugin.getPluginConfig().getStringList("salaries.list")
                  .contains(salaryName)) {
               sender.sendMessage(ChatColor.GREEN + CashFlow.TAG
                     + " Enabling salary - " + ChatColor.GOLD + salaryName);
         ...
         if(args.length == 2) {
            ...
            if(plugin.getPluginConfig().getStringList("salaries.list")
               ...
               sender.sendMessage(ChatColor.YELLOW + CashFlow.TAG
                     + " Disabling salary - " + ChatColor.AQUA + salaryName);
   ...
   private void showHelp(CommandSender sender) {
      ...
      sender.sendMessage(ChatColor.AQUA + "/salary " + ChatColor.YELLOW
            ...
            + "- Creates a salary.");
      sender.sendMessage(ChatColor.AQUA + "/salary " + ChatColor.YELLOW
            + "delete <salaryname> " + ChatColor.GREEN + "- Deletes a salary.");
