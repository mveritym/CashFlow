...
public class CashFlowListener implements Listener {
    ...
    * onPlayerQuit(final PlayerQuitEvent event) { // Update last on time for
    * player and to check it cf.getDatabaseHandler().standardQuery( "UPDATE " +
    * config.tablePrefix + "cashflow SET laston='" + System.currentTimeMillis()
    * + "', check='1' WHERE playername='" + event.getPlayer().getName() + "';");
    * }
    */
         ...
         if(!has) {
            if(config.debug) {
               plugin.getLogger().warning(
                     CashFlow.TAG + " PlayerJoin - add new player");
            }
            // Add to master list
            query = "INSERT INTO " + Table.CASHFLOW.getName()
                  + " (playername) VALUES('" + event.getPlayer().getName()
                  + "');";
